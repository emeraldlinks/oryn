      ...
      switch (actionType) {
        ...
        default:
          if (actionType.startsWith("plugin:")) {
            ...
            if (parts.length < 3) {
              throw new Error(`Invalid plugin action format: ${actionType}. Expected plugin:<pluginName>:<actionName>`);
            }
            const pluginName = parts[1];
            const actionName = parts.slice(2).join(":");
            const aiTools = pluginRegistry.getAITools(wsId);
            const matchedTool = aiTools.find(t => t.name === actionName && t.pluginId !== undefined);
            if (!matchedTool) {
              throw new Error(`Plugin AI tool "${actionName}" not found in plugin "${pluginName}"`);
            }
            const baseUrl = process.env.NEXT_PUBLIC_APP_URL || `http://localhost:${process.env.PORT || 3000}`;
            const response = await fetch(
              `${baseUrl}/api/plugins/${pluginName}/actions/${matchedTool.config.handler}`,
              {
                method: "POST",
            ...
            if (!response.ok) {
              ...
              throw new Error((errBody as any).error || `Plugin action failed with status ${response.status}`);
            }
